...
export class FavoritesPage extends Base {
    ...
    load(callback?){
        if(this.user.is_loggedin){
            // this.show_loader();
            this.__loading = true;
            this.api.post('get_likes', {type: this.type}).subscribe(data => {
                // this.hide_loader();
    ...
    onDelete(item){
        ...
        this.api.post('like_delete', { id: ID, type: this.type }).subscribe(data => {
            console.log('deleted');
        });
    }
    ...
    onInfinite(event){
        if(this.user.is_loggedin){
            if(this.page < this.total_pages){
                this.api.post('get_likes', {type: this.type, page: this.page+1}).subscribe(data => {

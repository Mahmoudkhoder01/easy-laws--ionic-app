...
export class RecordPage extends Base {
    ...
    save(){
        if(this.check()){
            this.show_loader();
            this.api.post('set_request_m4a', this.item, 240000).subscribe(data => {
                this.hide_loader();
                if(data.valid == 'YES'){
                    this.alert(this.trans.SUCCESS, this.trans.THANK_YOU_RECORDING);
    ...
    stop_recording(event){
        ...
        this.media.finish().then((data: any) => {

...
export class User {
    ...
    get user() {
        return this.storage.get("user") ? this.storage.get("user") : false;
    }
    get user_id() {
    ...
    set_browsing_history(args) {
        ...
        if (args.question_id && args.title) {
            ...
            if (this.user_id)
                this.api
                    .post("set_browsing_history", args)
                    .subscribe((data) => {});
        }
    }
    ...
    set search_history(keyword) {
        ...
        if (this.user_id)
            this.api
                .post("set_search_history", { keyword: keyword })
                .subscribe((data) => {});
    }
    get search_history() {
    ...
    forgot(email = "") {
        return new Promise((resolve, reject) => {
            this.api.post("forgot", { email: email }).subscribe(
                (data) => {
                    if (data.valid == "YES") {
                        // this.storage.set('user', data.results);
    ...
    signup(userObj) {
        return new Promise((resolve, reject) => {
            this.api.post("signup", userObj).subscribe(
                (data) => {
                    if (data.valid == "YES") {
                        // this.storage.set('user', data.results);
                        resolve(data.results);
    ...
    login(userObj) {
        return new Promise((resolve, reject) => {
            this.api.post("login", userObj).subscribe(
                (data) => {
                    if (data.valid == "YES") {
                        this.storage.set("user", data.results);
    ...
    setup_FB(response) {
        return new Promise((resolve, reject) => {
            ...
            this.fb
                .api("/me?fields=name,email,gender,birthday", [])
                .then((user) => {
                    let userObj = {
                        name: user.name,
                        email: user.email,

...
export class LAmeta {
    ...
    defaultBorder: string = 'border:0px solid white';
...
export class LetterAvatar implements OnInit, OnDestroy, AfterViewInit {
    ...
    @Input('avatar-font-size') fontsize: string = '20';
    @Input('avatar-height') height: string;
    @Input('avatar-width') width: string;
    @Input('avatar-font-weight') fontWeight: string = '600';
    ...
    @Input('avatar-char-count') charCount: string = '2';
...
//        console.log('element data:' + this.letteravatar['data']);
    ...
    private createAvatar(): void {
        var c = '';
        if (this.charCount == '2') {
            var _data = getFirstAndLastName(this.data);
            if (_data) {
                c = _data;
            } else {
                c = this.data.substring(0, +this.charCount);
            }
        } else {
            c = this.data.substring(0, +this.charCount);
        }

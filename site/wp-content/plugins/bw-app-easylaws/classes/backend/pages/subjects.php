...
class App_Subjects {
	...
	public function __construct() {
		...
		$this->plural = 'Subjects';
		$this->slug = 'app-subjects';
				...
				<form method="get">
					...
					jQuery(document).ready(function($){
						var tree = $("#TREE"),
							...
							e_all = $("#e_all"),
							c_all = $("#c_all"),
							data = \''.app_json_tree('subjects', '', true).'\';
	...
	public function addnew(){
		...
		if(isset($_POST['submit'])){
			...
			if(!empty($err)){
				...
				if(!is_numeric($insert_vals['parent'])){
					$insert_vals['parent'] = 0;
				}
				// $insert_vals['keywords'] = $insert_vals['keywords'] ? implode(',', $insert_vals['keywords']) : '';
				$insert_vals['keywords'] = __fix_spaces($insert_vals['keywords']);

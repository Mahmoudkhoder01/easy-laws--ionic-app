...
class App_Tags_List_Table extends WP_List_Table {
	public $base_link;

    public function prepare_items($per_page = 20, $base_link = '') {
    	$this->base_link = $base_link;
        $table = DB()->prefix . 'app_tags';
    ...
    public function single_row($row) {
        ...
        $prx = DB()->prefix.'app_';
        $qn = DB()->get_var("SELECT count(*) from {$prx}questions where FIND_IN_SET({$row->ID}, tags)>0");
        $questions = '---';
        if($qn){
            $questions = '<a href="admin.php?page=app-questions&app_filter_tag='.$row->ID.'">'.$qn.' (view)</a>';
        }

        $author = $row->author ? get_userdata($row->author)->user_nicename : '---';
        ...
        foreach ($columns as $column_name => $column_display_name) {
            ...
            switch ($column_name) {
                ...
                case 'questions': $r.= "<td $atts>$questions</td>"; break;
                case 'author': $r.= "<td $atts>$author</td>"; break;
                case 'dates': $r.= "<td $atts>$dates</td>"; break;
                default: $r.= "<td $atts>".apply_filters('manage_tags_custom_column', '', $column_name, $row->ID)."</td>";
            }
        }
        $r.= '</tr>';

...
*	------------------------------------------------
*	Bitwize Framework
* 	Copyright Bitwize SARL 2013 - http://www.bitwize.com.lb
*
*/

(function($) {

	$("#trans_count").live("click", function(e){
        e.preventDefault();
        var el = $(this),
        	ao = el.text(),
        	al = "Loading...",
        ...
        el.text(al);
        sd.text("");
        $.post(ajaxurl,{action:"bw_get_transients_count"}, function(data){
        	el.text(ao);
        	sd.text(data);
        });
    });

    $(".bwd_repair_db").live("click", function(e){
        e.preventDefault();
        var el = $(this),
        	ao = el.text(),
        	al = "Loading...",
        	sd = $("#bwd_repair_display"),
        	repair = 1;

        el.text(al);
        sd.text("").hide();

        if(el.hasClass('optimize')){
        	repair = 2;
        }
        $.post(ajaxurl,{'action':'bwd_repair_db', 'repair': repair}, function(data){
        	el.text(ao);
        	sd.empty().html(data).slideDown();
        });
    });

    $(".bwd_backup_db").live("click", function(e){
        e.preventDefault();
        var el = $(this),
        	ao = el.text(),
        	al = "Loading...",
        	sd = $("#bwd_backup_display");

        el.text(al);
        sd.text("").hide();
        $.post(ajaxurl,{'action':'bwd_backup_db'}, function(data){
        	el.text(ao);
        	sd.empty().html(data).slideDown();
        });
    });

	// jQuery("#submitdiv").css('z-index','100').hcSticky({noContainer:true, top:35});
	jQuery(".media-modal").css('z-index','101');
	...
	jQuery('.bw_upload_image_button').click(function() {
		//alert("WA#####");
		formfield = jQuery(this).siblings('.bw_upload_image');
		preview = jQuery(this).siblings('.bw_preview_image');
		tb_show('', 'media-upload.php?type=image&TB_iframe=true');
		window.send_to_editor = function(html) {
			imgurl = jQuery('img',html).attr('src');
			classes = jQuery('img', html).attr('class');
	...
	$('.plugin-version-author-uri').each(function(){
		var t = $(this),
			txt = t.text(),
			arr = txt.split('|'),
			nt = arr[0];

...
class Simple_Weather_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$data_file_days = BW_Weather::get_weather_file( $location, $latitude, $longitude, $days, $units, 0, $lang, $api, $interval, $timeout );
		...
		$data_file_current = BW_Weather::get_weather_file( $location, $latitude, $longitude, 0, $units, 1, $lang, $api, $interval, $timeout );
		...
		<div class="simple-weather-widget" style="background-color: <?php echo ($bg != null) ? $bg : 'none'; ?>; color: <?php echo isset($text) ? $text : 'inherit'; ?>; <?php if(isset($bg)) echo 'padding: 10px;' ?>;">
			...
			<div class="temp">
				...
				<span class="details">
					...
					<?php _e('Wind' , BW_TD) ?> <small>(<?php echo BW_Weather::get_wind_direction($json_data_current['wind']['deg']) ?>)</small>: <em class="float-right"><?php echo ($units == 'metric') ?  ceil($json_data_current['wind']['speed'] * 3.6).'<small>kph</small>' : ceil($json_data_current['wind']['speed'] * 3.6 / 1.609344).'<small>mph</small>' ?></em><br>
				</span>
			</div>
			...
			<div class="simple-weather-table" style="border-color: rgba(<?php echo $text ? $this->hex2rgb( $text ) : 'inherit'; ?>, .3);">
				...
				<div>
					...
					<div><i data-sw-icon="<?php echo BW_Weather::get_weather_icon($value['weather'][0]['id']) ?>"></i></div>
	...
	function form( $instance ) {
		...
		<div class="widget-content">
            <p>
                <label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e('Widget Title:', BW_TD); ?></label>
                <input type="text" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" value="<?php echo $instance['title']; ?>" class="widefat" />
            </p>
            <p>
                <label for="<?php echo $this->get_field_id( 'location-type' ); ?>"><?php _e('Location Type:', BW_TD); ?></label>
            ...
            <p class="loc-cont" style="display: <?php echo ($instance['location-type'] == 'location') ? 'block' : 'none'; ?>;">
                <label for="<?php echo $this->get_field_id( 'location' ); ?>"><?php _e('Location:', BW_TD); ?></label>
                ...
                <small style="color: gray;">Example: Beirut, LB</small>
            </p>
            <p class="lat-cont"  style="display: <?php echo ($instance['location-type'] == 'coordinates') ? 'block' : 'none'; ?>;">
                <label for="<?php echo $this->get_field_id( 'latitude' ); ?>"><?php _e('Location Latitude:', BW_TD); ?></label>
            ...
            <p class="lon-cont"  style="display: <?php echo ($instance['location-type'] == 'coordinates') ? 'block' : 'none'; ?>;">
                <label for="<?php echo $this->get_field_id( 'longitude' ); ?>"><?php _e('Location Longitude:', BW_TD); ?></label>
            ...
            <p>
                <label for="<?php echo $this->get_field_id( 'units' ); ?>"><?php _e('Units:', BW_TD); ?></label>
            ...
            <p>
                <label for="<?php echo $this->get_field_id( 'days' ); ?>"><?php _e('Days of forecast:', BW_TD); ?></label>
            ...
            <p>
                <label for="<?php echo $this->get_field_id( 'interval' ); ?>"><?php _e('Weather Check Interval:', BW_TD); ?></label>
            ...
            <p>
                <label for="<?php echo $this->get_field_id( 'timeout' ); ?>"><?php _e('Response Timeout:', BW_TD); ?></label>
            ...
            <p>
                <label for="<?php echo $this->get_field_id( 'lang' ); ?>"><?php _e('Language:', BW_TD); ?></label>
            ...
            <h4><?php _e('Widget Styling' , BW_TD) ?></h4>
            ...
            <p>
            	<label><?php _e('Background Color' , BW_TD) ?></label><br>
            ...
            <p>
            	<label><?php _e('Text Color' , BW_TD) ?></label><br>
            ...
            <p>
                <label for="<?php echo $this->get_field_id( 'api' ); ?>"><?php _e('API Key: (optional)', BW_TD); ?></label>
                ...
                <small><?php _e('For better performance it is recommended that you use an API key. To get access to weather API you need an API key whatever account you chose from Free to Enterprise. <a href="http://openweathermap.org/appid" target="_blank">Generate API Key</a>', BW_TD); ?></small>
            </p>
		</div>
	<?php
	}

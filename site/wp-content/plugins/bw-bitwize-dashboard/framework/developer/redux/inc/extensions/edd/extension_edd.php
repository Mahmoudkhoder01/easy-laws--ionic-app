    ...
    class ReduxFramework_extension_edd extends ReduxFramework {
      ...
      function register($field) {
        if ( $field['mode'] == "theme" ) {
          ...
          if ( !empty( $this->parent->options[$field['id']]['license'] ) && ucfirst( $this->parent->options[$field['id']]['status'] ) == __('Valid', BW_TD) ) {
      ...
      function license_call($array = array()) {
        ...
        if( $license_data->license == 'site_inactive' ) {
          ...
          set_transient( 'redux_edd_license_'.$_POST['data']['field_id'] . '_valid', __('Not Activated', BW_TD), 3600 * 24 );
        } else if( $license_data->license == 'deactivated' ) {
          $result = json_encode(array('status'=>'Deactivated', 'response'=>$license_data->license));
          set_transient( 'redux_edd_license_'.$_POST['data']['field_id'] . '_valid', __('Deactivated', BW_TD), 3600 * 24 );
        } else if( $license_data->license == 'valid' ) {
          $result = json_encode(array('status'=>'Valid', 'response'=>$license_data->license));
          set_transient( 'redux_edd_license_'.$_POST['data']['field_id'] . '_valid', __('Valid', BW_TD), 3600 * 24 );
        } else {
          // Change status
          $result = json_encode(array('status'=>__('Not Valid', BW_TD), 'response'=>$license_data->license));

	...
	class ReduxFramework_edd_license extends ReduxFramework {
		...
		public function render() {
			...
			if ( !empty( $this->value['license'] ) ) {
				...
				$this->value['status'] = __('Not Activated', BW_TD);
			}
			$this->value['status'] = ucfirst( $this->value['status'] );

			if ( $this->value['status'] == __('Deactivated', BW_TD) ) {
				$noticeClasses = 'redux-warning redux-info-field';
			} else if ( $this->value['status'] == __('Valid', BW_TD) ) {
			...
			echo '<b>' . __('License Status', BW_TD) . ': <span id="' . $this->field['id'] . '-status_notice">'.ucfirst($this->value['status']).'</span></b></div>';
			...
			echo '<a href="#" data-id="'.$this->field['id'].'" class="button button-primary redux-EDDAction hide" data-edd_action="check_license">'.__('Verify License', BW_TD).'</a>';
			$hide = "";
			if ($this->value['status'] == "Valid") {
				$hide = " hide";
			}
			echo '&nbsp; <a href="#" id="'.$this->field['id'].'-activate" data-id="'.$this->field['id'].'" class="button button-primary redux-EDDAction'.$hide.'" data-edd_action="activate_license">'.__('Activate License', BW_TD).'</a>';
			$hide = "";
			if ($this->value['status'] != "Valid") {
				$hide = " hide";
			}
			echo '&nbsp; <a href="#" id="'.$this->field['id'].'-deactivate" data-id="'.$this->field['id'].'" class="button button-secondary redux-EDDAction'.$hide.'" data-edd_action="deactivate_license">'.__('Deactivate License', BW_TD).'</a>';

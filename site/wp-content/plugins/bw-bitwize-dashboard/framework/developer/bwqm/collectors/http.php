...
class BWQM_Collector_HTTP extends BWQM_Collector {

	public $id = 'http';

	public function name() {
		return __( 'HTTP Requests', BW_TD );
	}
	...
	public function filter_http_request_args( array $args, $url ) {
		...
		if ( isset( $args['_bwqm_key'] ) ) {
			...
			$args['_bwqm_original_key'] = $args['_bwqm_key'];
			$start = $this->data['http'][$args['_bwqm_key']]['start'];
		} else {
	...
	public function action_http_api_debug( $response, $action, $class, $args, $url ) {
		...
		switch ( $action ) {
			...
			case 'response':
				...
				if ( !empty( $class ) ) {
					$this->data['http'][$args['_bwqm_key']]['transport'] = str_replace( 'wp_http_', '', strtolower( $class ) );
				} else {
					$this->data['http'][$args['_bwqm_key']]['transport'] = null;
				}
	...
	public function log_http_response( $response, array $args, $url ) {
		$this->data['http'][$args['_bwqm_key']]['end']      = microtime( true );
		$this->data['http'][$args['_bwqm_key']]['response'] = $response;
		if ( isset( $args['_bwqm_original_key'] ) ) {
			$this->data['http'][$args['_bwqm_original_key']]['end']      = $this->data['http'][$args['_bwqm_original_key']]['start'];
			$this->data['http'][$args['_bwqm_original_key']]['response'] = new WP_Error( 'http_request_not_executed', __( 'Request not executed due to a filter on pre_http_request', BW_TD ) );
		}
	}
	...
	public function process() {
		...
		$silent = apply_filters( 'bwqm/collect/silent_http_errors', array(
		...
		foreach ( $this->data['http'] as $key => & $http ) {
			...
			if ( !isset( $http['response'] ) ) {
				// Timed out
				$http['response'] = new WP_Error( 'http_request_timed_out', __( 'Request timed out', BW_TD ) );
...
BWQM_Collectors::add( new BWQM_Collector_HTTP );

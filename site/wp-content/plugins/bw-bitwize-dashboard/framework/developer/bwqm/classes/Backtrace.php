...
class BWQM_Backtrace {

	protected static $ignore_class = array(
		'wpdb'           => true,
		'BW_QueryMonitor'   => true,
		'ExtQuery'       => true,
		'W3_Db'          => true,
		'Debug_Bar_PHP'  => true,
	);
	...
	public function get_component() {
		...
		foreach ( $this->trace as $item ) {
			...
			try {
				...
				$comp = BWQM_Util::get_file_component( $file );
		...
		foreach ( BWQM_Util::get_file_dirs() as $type => $dir ) {
	...
	public function get_filtered_trace() {
		...
		if ( !isset( $this->filtered_trace ) ) {
			...
			if ( empty( $trace ) && !empty($this->trace) ) {
				$lowest                 = $this->trace[0];
				$file                   = BWQM_Util::standard_dir( $lowest['file'], '' );
	...
	public function filter_trace( array $trace ) {
		...
		if ( !self::$filtered and function_exists( 'did_action' ) and did_action( 'plugins_loaded' ) ) {
			...
			self::$ignore_class  = apply_filters( 'bwqm/trace/ignore_class',  self::$ignore_class );
			self::$ignore_method = apply_filters( 'bwqm/trace/ignore_method', self::$ignore_method );
			self::$ignore_func   = apply_filters( 'bwqm/trace/ignore_func',   self::$ignore_func );
			self::$show_args     = apply_filters( 'bwqm/trace/show_args',     self::$show_args );
			self::$filtered = true;
		...
		if ( isset( $trace['class'] ) ) {
			...
			if ( isset( self::$ignore_class[$trace['class']] ) ) {
				...
				$return = null;
			} else if ( 0 === strpos( $trace['class'], 'BWQM_' ) ) {
				$return = null;
			} else {
			...
			if ( isset( self::$ignore_func[$trace['function']] ) ) {
				...
				if ( 'dir' === $show ) {
					if ( isset( $trace['args'][0] ) ) {
						$arg = BWQM_Util::standard_dir( $trace['args'][0], '~/' );
	...
	add_action( 'init', 'BW_QueryMonitor::symlink_warning' );
